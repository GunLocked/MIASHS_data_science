...
```{r }
#je teste sur mon jeu de données train
...
```{r}
set.seed(123)
rf_model <- randomForest(DIFF ~ .,
                         data = train,
                         ntree = 500,
                         mtry = 4,
                         maxnodes = 5, #meilleur resultat avec 5
                         importance = TRUE,
                         )
...
```
```{r}
# Suppression éventuelle de R32 avant de prédire
# train <- train[, !names(train) %in% c("R32")]
# test <- test[, !names(test) %in% c("R32")]
